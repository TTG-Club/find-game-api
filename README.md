...
Каждый ответ об игре несёт `approvedPlayers` — сколько игроков принято в ближайшую предстоящую
сессию игры. Ближайшая — самая ранняя сессия в статусе `SCHEDULED`, у которой время ещё не прошло;
наборы с открытой датой идут после датированных. Поле только для чтения: в игре оно не хранится и
считается по заявкам со статусом `APPROVED`. Вместе с `maxPlayers` и `playersToStart` оно даёт
занятость мест для карточки каталога: сколько мест всего, сколько занято и сколько нужно для старта.
