...
Каждый ответ об игре несёт `approvedPlayers` — сколько разных игроков уже принято в её сессии.
Поле только для чтения: в игре оно не хранится и считается по заявкам со статусом `APPROVED`.
Вместе с `maxPlayers` оно даёт занятость мест для карточки каталога.
